...
using static_eng::utils::get_config;
using static_eng::utils::sanitize;
using static_eng::utils::split;

TEST(CONFIG_LOADER, SanitizeString) {
  // By default should drop leading / trailing spaces
  EXPECT_EQ(sanitize("no_filter_needed"), "no_filter_needed");
  EXPECT_EQ(sanitize("no filter needed"), "no filter needed");
  EXPECT_EQ(sanitize("    lead_filter_needed"), "lead_filter_needed");
  EXPECT_EQ(sanitize("    trail_filter_needed"), "trail_filter_needed");
  EXPECT_EQ(sanitize("  spaces_each_side  "), "spaces_each_side");
  EXPECT_EQ(sanitize("  spaces between words  "), "spaces between words");
  EXPECT_EQ(sanitize("@custom@char@@", '@'), "custom@char");
}
...
TEST(CONFIG_LOADER, KV_STRING_PAIRS) { get_config<"log_level">(); }